...
只使用speed选项，测试所有算法的性能
	...
	openssl speed
有两组结果，第一组表示每种算法一秒可以处理多少字节的数据，第二组表示一次签名／验证花费的时间。  
...
使用s_time选项可以测试连接性能，直接调用会运行30秒，使用任意的加密算法，统计每秒钟通过SSL握手建立的连接数，结果分为新会话和重用会话两部分：
...
如果你没有可测的启用SSL的服务端，可以使用s_server选项来模拟一个。
...
首先要决定你是否想加密你的密钥，这意味着你的密钥被密码保护着。  
好处是更加安全，有人拿走了你的密钥也是无法使用的。坏处是需要把密码保存在某个文件中或每次启动服务时手动输入。  
我个人倾向不加密密钥，不用每次手动输入密码。（当你厌烦输入密码时，要知道去除密码保护也是可以的。）  
下面的这个例子会生成一个mycert.pem证书文件，里面包含私钥公钥相关信息。这个证书的有效期是365天，不加密密钥(-ndoes决定的)。

	openssl req \
		-x509 -nodes -days 365 -sha256 \
		-newkey rsa:2048 -keyout mycert.pem -out mycert.pem
使用这个命令后，会要求你回答一系列的问题：国家，省市，公司，职位等等。其中回答“Common Name”这个问题要注意点，你可能会填写主机名或服务端域名，这很重要，如果服务器的真实地址是mybox.mydomain.com，但用户是通过www.mydomain.com来访问的，那么你就应填写用户访问的地址。  
你可以通过-subj选项来直接写入这些回答，在生成的证书中惟一有用的就是Common Name(CN)。
...
向认证机构(比如VeriSign)申请签名证书是相当繁杂和官僚的，在创建证书请求前你要先准备好需要的书面文件(营业执照等)。  
上一节说到，你需要考虑是否加密密钥，下面的例子假设不加密，最后会得到两个文件：私钥文件mykey.pem，证书请求文件myreq.pem。
...
怎么得到远端的证书？  
只要配合使用openssl和sed命令，就能取到远端的证书(当然不包含私钥)。
